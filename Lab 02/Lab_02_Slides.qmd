...
-   Imagine you start with a line [(1D)]{style="color:red;"}. You only need a few points to cover the length of the line, since it's just a straight path.
...
-   Now, think about a square [(2D)]{style="color:red;"}. Instead of just covering a line, you now need to fill up an entire area. You need more points to cover both the width and height of the square.
...
-   Next, picture a cube [(3D)]{style="color:red;"}. To fill the volume of the cube, you need even more points because you’re now working in three directions: length, width, and height.
...
Locate the `Ozone` data, which records Los Angeles ozone pollution levels in 1976. Follow the instructions below and submit a PDF of your results to the Canvas ![Canvas](Canvas-Icon.png){width="6%"}. Be sure to include relevant figures.
...
[Source: R Documentation `Ozone {mlbench}`](https://search.r-project.org/CRAN/refmans/mlbench/html/Ozone.html)
:::
...
::: fragment
[The histogram appears more consistent with a gamma distribution.]{style="color:red;"}
:::
